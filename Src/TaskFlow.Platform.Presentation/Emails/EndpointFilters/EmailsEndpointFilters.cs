...
namespace TaskFlow.Platform.Presentation.Emails.EndpointFilters;

public sealed class EmailsEndpointFilters : IEndpointFilter
{
    public async ValueTask<object?> InvokeAsync(EndpointFilterInvocationContext context, EndpointFilterDelegate next)
    {
        ...
        {
            return Results.Json(
                new { errors = new[] { new { message = "Unauthorized access" } } },
        ...
        {
            return Results.Json(
                new { errors = ex.Errors.Select(e => new { message = e }).ToArray() },
                statusCode: StatusCodes.Status400BadRequest);
        }
        catch (Exception ex)
        {
